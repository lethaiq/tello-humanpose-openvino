"""
tello_selfie_assistant.py : Use the Tello drone as an "selfie air stick" (selfie assistant)
Relies on tellopy (for interaction with the Tello drone) 
and Openvino (to run inference on a Human Pose Recognition model)
...
from modules.CameraMorse import CameraMorse, RollingGraph
...
def openpose_worker():
    ...
    """
    log.info(f"Worker process pid={os.getpid()}")
    ...
    while True:
        ...
        if tello.use_sound: tello.sound_player.play()
        cv2.waitKey(1)

def main(use_multiprocessing=False, device="CPU", log_level=None, model_input_height=256, use_sound=True, kbd_layout="QWERTY"):
    """ 
...
class TelloController(object):
    ...
    def __init__(self, use_face_tracking=True, 
        ...
        if not self.use_multiprocessing:
            if self.use_sound: self.init_sounds()
            self.init_controls()
        ...
        if log_level is not None:
            ...
            ch.setFormatter(logging.Formatter(fmt='%(asctime)s.%(msecs)03d - %(name)s - %(levelname)s - %(message)s',
                            datefmt="%H:%M:%S"))
            log.addHandler(ch)
            ch2 = logging.FileHandler("tello.log",mode='w')
            ch2.setLevel(log_level)
            ch2.setFormatter(logging.Formatter(fmt='%(asctime)s.%(msecs)03d - %(name)s - %(levelname)s - %(message)s',
                            datefmt="%H:%M:%S"))
            log.addHandler(ch2)
            self.drone.set_loglevel(log_level)
    ...
    def palm_land(self):
        ...
        if self.use_sound: self.sound_player.play("palm landing")
    ...
    def toggle_openpose(self):
        ...
        if not self.use_openpose:
            ...
            self.toggle_tracking(tracking=False)
        log.info('HUMAN POSE '+("ON" if self.use_openpose else "OFF"))
...
if __name__ == '__main__':
    ...
    ap.add_argument("-1", "--monoprocess", action='store_true', help="use 1 process (instead of 2)")
    ap.add_argument("--no_sound", action='store_true', help="Desactivate sound")
    ap.add_argument("-k", "--keyboard_layout", default="QWERTY", help="Your keyboard layout (QWERTY or AZERTY) (default=%(default)s)")
    ap.add_argument("-s", "--height_size", help='Network input layer height size. The smaller the faster (default=%(default)i)', type=int, default=256)

    args=ap.parse_args()
    assert args.keyboard_layout == "QWERTY" or args.keyboard_layout == "AZERTY"

    main(use_multiprocessing=not args.monoprocess, log_level=args.log_level, model_input_height=args.height_size, use_sound= not args.no_sound, kbd_layout=args.keyboard_layout)